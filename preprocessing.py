...
class Text:
    ...
    def punct_marks_to_slash(self):
        self.text = re.sub(r"[,:—―–]", ' / ', self.text)
        self.text = re.sub(r"[;…?!]", ' // ', self.text)
        self.text = re.sub(r"[()\"'«»„“”]", '', self.text)
        self.text = re.sub(r"u+00A0", ' ', self.text)
    ...
    def tokenize_text(self):
        self.text = word_tokenize(self.text, language="russian")
        for i in self.text:
            if i == '.':
        ...
        oldsign_index = 0
        sign_index = 1
        while sign_index in range(len(self.text)):
            if self.text[oldsign_index] in ['//', '/'] and self.text[sign_index] in ['//', '/']:
                self.text.pop(oldsign_index)
            else:
                oldsign_index += 1
                sign_index += 1
...
stroka = Text()
stroka.get_user_input()
stroka.text = accentuate(stroka.text, wordforms, lemmas)
stroka.punct_marks_to_slash()
stroka.tokenize_text()
print(stroka.text)
